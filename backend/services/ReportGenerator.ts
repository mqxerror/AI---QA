...
class ReportGenerator {
   ...
   */
  static async generateReport(options: ReportOptions): Promise<string> {
   ...
   * Add report header
   */
  private static addHeader(doc: PDFKit.PDFDocument, testRun: TestRun): void {
    // Title
    doc.fontSize(24)
       .fillColor('#1f2937')
   ...
   */
  private static addTestSummary(doc: PDFKit.PDFDocument, testRun: TestRun): void {
    doc.fontSize(16)
       .fillColor('#1f2937')
   ...
   */
  private static addSmokeTestResults(doc: PDFKit.PDFDocument, results: any[]): void {
    doc.fontSize(16)
       .fillColor('#1f2937')
   ...
   */
  private static addPerformanceResults(doc: PDFKit.PDFDocument, metrics: any): void {
    if (!metrics) return;
   ...
   */
  private static addPixelAuditResults(doc: PDFKit.PDFDocument, results: any[]): void {
    doc.fontSize(16)
       .fillColor('#1f2937')
   ...
   */
  private static addLoadTestResults(doc: PDFKit.PDFDocument, results: any): void {
    if (!results) return;
   ...
   */
  private static addAccessibilityResults(doc: PDFKit.PDFDocument, results: any[]): void {
    doc.fontSize(16)
       .fillColor('#1f2937')
   ...
   */
  private static addSecurityScanResults(doc: PDFKit.PDFDocument, results: any): void {
    if (!results) return;
    ...
    if (results.violations) {
      ...
      violations.slice(0, 15).forEach((violation: any, index: number) => {
   ...
   */
  private static addSEOAuditResults(doc: PDFKit.PDFDocument, results: any): void {
    if (!results) return;
    ...
    if (results.issues) {
      ...
      issues.slice(0, 15).forEach((issue: any, index: number) => {
        if (index > 0 && index % 6 === 0) {
          doc.addPage();
        }
   ...
   */
  private static addVisualRegressionResults(doc: PDFKit.PDFDocument, results: any): void {
    if (!results) return;
    ...
    if (results.comparisons) {
      ...
      comparisons.forEach((comp: any, index: number) => {
        ...
        try {
          // Current screenshot
          const screenshotPath = path.join(screenshotsDir, comp.screenshot_path);
          ...
          if (comp.baseline_path) {
            const baselinePath = path.join(baselineDir, comp.baseline_path);
          ...
          if (!comp.is_baseline_run && comp.diff_path) {
            const diffPath = path.join(diffDir, comp.diff_path);
   ...
   */
  private static addFooter(_doc: PDFKit.PDFDocument): void {
   ...
   * Ensure reports directory exists
   */
  private static ensureReportsDirectory(): void {
