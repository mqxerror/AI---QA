...
class SEOAuditor {
     ...
     * Check meta tags
     */
    static async checkMetaTags(page, issues) {
     ...
     */
    static async checkOpenGraphTags(page, issues) {
     ...
     */
    static async checkTwitterCardTags(page, issues) {
        const cardType = await page.$eval('meta[name="twitter:card"]', (el) => el.content).catch(() => null);
        const twitterTitle = await page.$eval('meta[name="twitter:title"]', (el) => el.content).catch(() => null);
        const twitterDescription = await page.$eval('meta[name="twitter:description"]', (el) => el.content).catch(() => null);
     ...
     */
    static async checkStructuredData(page, issues) {
        ...
        const schemaTypes = jsonLdScripts.map((data) => data['@type'] || 'Unknown').filter((type) => type !== 'Unknown');
     ...
     */
    static async checkHeadings(page, issues) {
        const h1Elements = await page.$$eval('h1', (elements) => elements.map(el => el.textContent?.trim() || ''));
     ...
     */
    static async checkImages(page, issues) {
     ...
     * Check links
     */
    static async checkLinks(page, baseUrl, issues) {
        const linkData = await page.$$eval('a[href]', (links, baseUrl) => {
     ...
     */
    static async checkTechnicalSEO(page, url, issues) {
        ...
        try {
            const robotsResponse = await axios_1.default.get(robotsTxtUrl, { timeout: 5000, validateStatus: () => true });
        ...
        try {
            const sitemapResponse = await axios_1.default.get(sitemapUrl, { timeout: 5000, validateStatus: () => true });
     ...
     * Calculate overall SEO score (0-100)
     */
    static calculateSEOScore(summary) {
        let score = 100;
...
exports.default = SEOAuditor;
//# sourceMappingURL=SEOAuditor.js.map