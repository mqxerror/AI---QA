...
class SecurityScanner {
   ...
   */
  static async runScan(url: string): Promise<SecurityScanResult> {
  ...
  private static async checkSSL(url: string): Promise<{
    ...
    try {
      ...
      const sslInfo = await sslChecker(hostname) as any;
   ...
   */
  private static async checkSecurityHeaders(url: string): Promise<{
    headers: any;
    violations: SecurityViolation[];
  }> {
    const violations: SecurityViolation[] = [];
   ...
   */
  private static async checkMixedContent(url: string): Promise<SecurityViolation[]> {
    const violations: SecurityViolation[] = [];
   ...
   */
  private static async checkCommonVulnerabilities(url: string): Promise<SecurityViolation[]> {
    const violations: SecurityViolation[] = [];
   ...
   * Calculate overall security score (0-100)
   */
  private static calculateSecurityScore(summary: {
    critical: number;
    high: number;
    medium: number;
    low: number;
